...
import { ensureSamplePdf, getPdfInfo, extractPageAsPdf, replacePdf, getPdfBytes } from '@/lib/pdfStorage'
...
async function seedAdmin() {
  ...
  if (!existing) {
    const password = await hashPassword(process.env.ADMIN_PASSWORD || 'Admin@123')
    ...
    await db.collection('users').updateOne({ id: existing.id }, { $set: { role: 'admin', purchasedBook: true } })
  }
  seeded = true
}
...
async function handler(request, { params }) {
  ...
  try {
    ...
    if (route === '/auth/register' && method === 'POST') {
      ...
      if (!parse.success) return json({ error: 'Invalid input', details: parse.error.flatten() }, 400)
    ...
    if (route === '/auth/forgot-password' && method === 'POST') {
      ...
      if (!parse.success) return json({ error: 'Invalid input' }, 400)
      const user = await db.collection('users').findOne({ email: parse.data.email })
      // Always respond ok to avoid user-enumeration
      if (user) {
        const token = crypto.randomBytes(32).toString('hex')
        const expiresAt = new Date(Date.now() + 1000 * 60 * 60) // 1h
        await db.collection('password_resets').insertOne({
          id: uuidv4(), userId: user.id, token, expiresAt, used: false, createdAt: new Date(),
        })
      ...
      return json({ ok: true, message: 'If the email exists, a reset link has been sent.' })
    }
    ...
    if (route === '/auth/reset-password' && method === 'POST') {
      ...
      if (new Date(rec.expiresAt) < new Date()) return json({ error: 'Token expired' }, 400)
      const hashed = await hashPassword(password)
      await db.collection('users').updateOne({ id: rec.userId }, { $set: { password: hashed } })
      await db.collection('password_resets').updateOne({ id: rec.id }, { $set: { used: true } })
      return json({ ok: true })
    }
    ...
    if (route === '/payment/create-order' && method === 'POST') {
      ...
      if (user.purchasedBook) return json({ error: 'Already purchased' }, 400)

      const price = Number(process.env.EBOOK_PRICE_INR || 149)
      const amount = price * 100 // paise
      const mock = process.env.MOCK_RAZORPAY === 'true' || !process.env.RAZORPAY_KEY_ID || !process.env.RAZORPAY_KEY_SECRET

      let order
      if (mock) {
        order = {
          id: 'order_mock_' + uuidv4().replace(/-/g, '').slice(0, 14),
          amount, currency: 'INR', status: 'created', mock: true,
        }
      } else {
        // Real Razorpay
        ...
        const rzpRes = await fetch('https://api.razorpay.com/v1/orders', {
          ...
          body: JSON.stringify({ amount, currency: 'INR', receipt: 'rcpt_' + user.id.slice(0, 10) }),
        })
        if (!rzpRes.ok) {
          const err = await rzpRes.text()
          return json({ error: 'Razorpay order failed', details: err }, 500)
        }
    ...
    if (route === '/payment/verify' && method === 'POST') {
      ...
      const purchase = await db.collection('purchases').findOne({ razorpayOrderId: razorpay_order_id, userId: user.id })
      ...
      await db.collection('purchases').updateOne(
        { id: purchase.id },
        { $set: { razorpayPaymentId: razorpay_payment_id || ('pay_mock_' + uuidv4().slice(0, 8)), status: 'paid', purchasedAt: new Date() } }
      )
      await db.collection('users').updateOne({ id: user.id }, { $set: { purchasedBook: true } })
      return json({ ok: true })
    }
    ...
    if (route === '/reader/info' && method === 'GET') {
      const auth = await requireUser()
      if (auth.error) return json({ error: auth.error }, auth.status)
      if (!auth.user.purchasedBook) return json({ error: 'Purchase required' }, 402)
      await ensureSamplePdf()
      const info = await getPdfInfo()
      return json({ ...info, title: process.env.EBOOK_TITLE, author: process.env.EBOOK_AUTHOR })
    }
    ...
    if (route.startsWith('/reader/page/') && method === 'GET') {
      ...
      if (!Number.isFinite(pageNum) || pageNum < 1) return json({ error: 'Invalid page' }, 400)
      try {
    ...
    if (route === '/admin/stats' && method === 'GET') {
      ...
      const totalPurchases = await db.collection('purchases').countDocuments({ status: 'paid' })
      const agg = await db.collection('purchases').aggregate([
        { $match: { status: 'paid' } },
        { $group: { _id: null, total: { $sum: '$amount' } } },
      ]).toArray()
      const totalRevenuePaise = agg[0]?.total || 0
      return json({ totalUsers, totalPurchases, totalRevenue: totalRevenuePaise / 100 })
    }
    ...
    if (route === '/admin/purchases' && method === 'GET') {
      const auth = await requireAdmin()
      if (auth.error) return json({ error: auth.error }, auth.status)
      const purchases = await db.collection('purchases').find({}).sort({ createdAt: -1 }).limit(2000).toArray()
    ...
    if (route === '/admin/export.csv' && method === 'GET') {
      const auth = await requireAdmin()
      if (auth.error) return json({ error: auth.error }, auth.status)
      const purchases = await db.collection('purchases').find({ status: 'paid' }).sort({ purchasedAt: -1 }).toArray()
      const userIds = [...new Set(purchases.map(p => p.userId))]
      const users = await db.collection('users').find({ id: { $in: userIds } }).toArray()
      const byId = Object.fromEntries(users.map(u => [u.id, u]))
      const rows = [['PurchaseID', 'UserName', 'UserEmail', 'RazorpayOrderId', 'RazorpayPaymentId', 'AmountINR', 'Status', 'PurchasedAt']]
    ...
    if (route === '/contact' && method === 'POST') {
      ...
      await db.collection('contacts').insertOne({
        id: uuidv4(), name, email, message, createdAt: new Date(),
      })
      return json({ ok: true })
    }
